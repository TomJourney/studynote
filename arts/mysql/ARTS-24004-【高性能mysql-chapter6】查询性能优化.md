...
readme：表数据量：film表-50w； actor表-50w； film_actor_rel表-10w； 	
...
补充1：straight_join 仅适用于 inner_join， 强行指定表关联顺序，即先执行左表，再执行右表； 

补充2：last_query_cost：获取mysql计算的当前查询的成本，其中成本的最小单位是随机读取1个大小为4K的数据页；



##### 【6.4.3.2.5】排序优化 

1. 当不能使用索引生成排序结果，mysql需要自己进行排序；
   1. 如果数据量小则在内存排序；
   2. 如果数据量大则需要使用磁盘；
   3. 但无论是否使用到磁盘，mysql统一称为文件排序；
2. 排序缓冲区：
   1. 如果排序的数据量小于排序缓冲区，则在内存进行快排；
   2. 如果排序缓冲区不够，则mysql会先把数据分块，对每个独立的块使用快排，然后把各个块的排序结果存放到磁盘上，然后将各个排序好的块进行合并；

3. mysql有2种排序算法：
   1. 两次传输排序（老版本使用）：读取行指针和需要排序的字段（仅排序列，不是select列），排序后，解雇排序结果读取锁需要的数据行（select字段）；
      1. 缺点：2次读取磁盘，2次io；
      2. 优化： 排序时，仅缓存了排序列，所以排序缓冲区可以放下更多的数据行，减少了磁盘文件排序的次数；
   2. 单次传输排序（新版本使用）：先读取查询所需要的所有列（select列），排序后，直接返回结果；
      1. 缺点：排序时，缓存了select列（不是排序列），排序缓冲区能放下的数据行会减少（相比两次传输排序），增加了磁盘排序的次数；
      2. 有点：仅1次io；
   3. 补充：如何选择排序算法啊？ 
      1. 当查询所有列的总长度不超过参数 max_length_for_sort_data时， 使用单次传输排序； 
      2. 当超过时，使用两次传输排序； 
4. mysql在进行文件排序时，**使用临时存储空间（临时表大小）可能比原有磁盘的表大得多**；
   1. 原因在于： mysql在排序时， 对每一个排序记录都会分配一个足够长的定长空间来存放；  （如varhcar列是分配其完整长度；如果使用utf-8字符，则为每个字符预留3个字节）
5. 关联表排序：分两种情况来处理表关联的排序：
   1. 如果排序的所有列来自关联的第1个表，则mysql在关联处理第1个表时就进行文件排序； explain查询执行计划结果，extra有 using filesort ；
   2. 除上述的其他情况：mysql都会先将关联结果存放到一个临时表，在关联结束后再做文件排序；执行结果extra显示 using temporary;using filesort；

### 【6.4.4】查询执行引擎 

1. mysql将生成查询对应的执行计划， mysql的查询计划执行引擎则根据这个计划来完成整个查询； 
2. 在根据执行计划逐步执行的过程中：有大量的操作需要通过调用存储引擎实现的接口来完成，这些接口称为 handler api 接口；
3. 查询中的每个表由一个handler实例表示； 实际上，mysql在优化阶段就为每个表创建一个handler实例， 优化器根据这些实例的接口可以获取表的相关信息，包括表的所有列名，索引统计信息等；
4. 存储引擎有着非常丰富的功能，底层接口就有几十个，这些接口像搭积木一样能够完成查询的大部分操作
   1. 如查询某个索引的第一行的接口；如查询某个索引条目的下一个条目的接口；



## 【6.5】mysql查询优化器的局限性

背景： mysql的万能的嵌套循环并不是对每种查询都是最优的；
